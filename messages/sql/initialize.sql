...
CREATE TABLE users ( name text PRIMARY KEY, connected boolean NOT NULL DEFAULT true);

DROP FUNCTION IF EXISTS notify_user_change();
CREATE FUNCTION notify_user_change()
RETURNS VOID AS $$
  SELECT pg_notify('callback', format_json('user_change', (SELECT COUNT(*) FROM users)::text));
...
CREATE TABLE messages ( id int PRIMARY KEY GENERATED ALWAYS AS IDENTITY, name text NOT NULL REFERENCES users, message text NOT NULL );

DROP FUNCTION IF EXISTS notify_message_change();
CREATE FUNCTION notify_message_change()
RETURNS VOID AS $$
  SELECT pg_notify('callback', format_json('message_change', array_to_json((SELECT ARRAY_AGG(m.message) OVER (ORDER BY m.id) FROM messages AS m ORDER BY m.id DESC LIMIT 5)) :: text));
$$ LANGUAGE SQL VOLATILE;
